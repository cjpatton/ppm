...
* `vdaf_start(j: Unsigned, verify_param, output_param, nonce, input_share) ->
  ...
  output by the setup algorithm (`verify_param`), the nonce provided by the
  environment (`nonce`; see {{run-vdaf}}), and one of the input shares generated
  by the client (`input_share`). Its outputs include the aggregator's initial
  state (`state`) and the outbound message it sends in the first round
  (`outbound_message`).
