...
* `vdaf_input(input_param, input) -> (input_shares: Vec[bytes], nonce)` is the
  ...
  is used by the aggregators to verify the input shares.
    ...
    [NOTE The nonce is used in prio3 to derive per-input query randomness from
    an initial key generated by the setup algorithm. (We'll need something
    similar for hits.) This could easily be folded into the the input shares and
    not exposed in the API. The reason to expose it in the API is that it's used
    in the main PPM protocol to sort reports and prevent replay attacks. Maybe
    there's another way to spell this?]
...
~~~
def vdaf_start(j, k_query_init, _, nonce, r_input_share):
